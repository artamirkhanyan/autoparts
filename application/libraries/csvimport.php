...
class Csvimport {
    ...
    //My flag if true - function return difference between template and 
    //uploaded csv headers, else - just return the array of the data
    private $return_difference = FALSE;
    
    public $headers_template = array(
        "part_number" => "Код",
        "description" => "Описание",
        "price" => "Цена",
        "inventory" => "Кол-во",
        "weight" => "Вес (кг)",
        "delivery" => "Дост-ка (дни)",
        "category" => "Марка"
    );
    
    public $my_column_headers = array();
   ...
   /**
     ...
     * @return  array
     */
    public function get_array($filepath=FALSE, $column_headers=FALSE, $detect_line_endings=TRUE, $initial_line=FALSE, $delimiter=FALSE, $return_difference=FALSE)
    {
        // File path
        if(! $filepath)
        {
        ...
        $this->_set_delimiter($this->detectDelimiter());
            ...
            if($row == $this->initial_line)
            {
                ...
                else // Parse first row for column_headers to use
                {
                    ...
                    if($return_difference){
                        $this->my_column_headers = $column_headers;
                        $check_result = $this->_check_column_headers_with_template($column_headers);
                        if($check_result){
                            return $check_result;
                        }
                    }
     ...
     * @return array
     */
    public function _check_column_headers_with_template($column_headers_map)
    {        
        $result_array = array_diff_assoc($column_headers_map, array_values($this->headers_template));
